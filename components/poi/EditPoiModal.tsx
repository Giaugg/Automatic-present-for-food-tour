...
export default function EditPoiModal({ open, onClose, poi, languages, onSuccess }: EditPoiModalProps) {
  const [activeTab, setActiveTab] = useState("vi-VN");
  
  // Khởi tạo state với cấu trúc mặc định để tránh lỗi null khi render lần đầu
  ...
  useEffect(() => {
    if (open && poi && languages?.length > 0) {
      // Ép kiểu translations từ server sang định dạng input
      const poiTranslations = (poi as any).all_translations || [];

      const mergedTranslations = languages.map((lang) => {
        const existingTrans = poiTranslations.find(
          (t: any) => (t.language_code || t.code) === lang.code
        );
        return {
      ...
      <div className="bg-white border-4 border-black rounded-[32px] w-full max-w-5xl overflow-hidden flex flex-col max-h-[92vh] shadow-[16px_16px_0px_0px_rgba(0,0,0,1)]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
               <div className="space-y-2">
                  ...
                  <div 
                    ...
                    className="aspect-video border-4 border-black rounded-3xl flex flex-col items-center justify-center cursor-pointer hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all overflow-hidden relative bg-gray-100 group"
                  >
                    <img 
                      src={
                        previewUrl || 
              ...
              {languages?.map((l) => (
                <button 
                  ...
                  className={`px-5 py-2 border-4 border-black rounded-2xl font-black text-[10px] whitespace-nowrap transition-all uppercase ${
                    activeTab === l.code ? 'bg-black text-white shadow-[4px_4px_0px_0px_rgba(16,185,129,1)]' : 'bg-white hover:bg-gray-100'
                  }`}
                >
                  {l.name}
                </button>
              ...
              {formData.translations.map((t, idx) => t.language_code === activeTab && (
                <div key={t.language_code} className="space-y-4 animate-in slide-in-from-right-2">
                  <div className="space-y-1">
                    ...
                    <input 
                      className="w-full border-4 border-black p-4 rounded-2xl font-black outline-none focus:bg-emerald-50 transition-all"
                      value={t.name}
                      onChange={e => {
                  ...
                  <div className="space-y-1">
                    ...
                    <textarea 
                      rows={8}
                      className="w-full border-4 border-black p-4 rounded-2xl font-black outline-none resize-none focus:bg-emerald-50 transition-all"
                      value={t.description}
                      onChange={e => {
                        const newTrans = [...formData.translations];
        ...
        <div className="p-8 border-t-4 border-black bg-gray-50 flex flex-col sm:flex-row gap-4">
          <button 
            disabled={isSubmitting}
            onClick={handleSave} 
            className="flex-[2] bg-black text-white p-5 rounded-[24px] font-black text-xl hover:bg-emerald-400 hover:text-black transition-all shadow-[8px_8px_0px_0px_rgba(0,0,0,0.2)] flex items-center justify-center gap-4 disabled:bg-gray-400 uppercase italic"
          >
