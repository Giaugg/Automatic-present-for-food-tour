...
export default function MapView() {
	...
	const handleProximityAudio = useCallback(
	...
	useEffect(() => {
		if (typeof window !== "undefined") {
			// Ép kiểu qua any để xóa thuộc tính cũ
	...
	// --- ICONS ---
	const userLocationIcon = isMounted
		? L.divIcon({
				...
				{useManual && (
					<div className="space-y-4 animate-in fade-in zoom-in-95">
						...
						<div className="p-4 bg-slate-900 rounded-[1.5rem] text-white shadow-inner">
							<div className="text-[8px] font-black uppercase text-slate-500 mb-1">
								Targeting
							</div>
							<div className="text-xs font-bold truncate">
								{targetPoi ? targetPoi.name : "Searching..."}
							</div>
							<div className="flex items-center gap-2 mt-2 text-[10px] font-mono text-blue-400">
								<Radius size={12} />{" "}
								{targetPoi ? `${(minDistance * 1000).toFixed(0)}m` : "---"}
							</div>
						</div>
						...
						<div className="space-y-2">
							<div className="flex items-center justify-between text-[9px] font-black uppercase text-slate-400">
								<div className="flex items-center gap-1">
									<Gauge size={12} /> Sensitivity
								</div>
								<span className="text-blue-600">
									{(moveSpeed * 100000).toFixed(0)}x
								</span>
							</div>
							<input
								type="range"
											...
											<div className="flex gap-3">
												<button
													onClick={() => {
														const url = getFullAudioUrl(poi.audio_url!);
														if (!url)
															return toast.error("Chưa có thuyết minh!");
